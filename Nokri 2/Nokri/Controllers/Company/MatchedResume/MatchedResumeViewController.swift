//
//  MatchedResumeViewController.swift
//  Nokri
//
...
class MatchedResumeViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let objData = NSKeyedUnarchiver.unarchiveObject(with: userData as! Data) as! [String: Any]
        let dataTabs = SplashRoot(fromDictionary: objData)
            self.title = dataTabs.data.extra.matched_resume
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "MatchedResumeTableViewCell", for: indexPath) as! MatchedResumeTableViewCell
        ...
        for itemDict in selectedActiveJob! {
            ...
            if let field_type_name = innerDict["field_type_name"] as? String{
                ...
                if field_type_name == "job_post" {
                    if let value = innerDict["value"] as? String {
                        cell.lblJobPostedVal.text = value
                    }
                    if let value = innerDict["key"] as? String {
                        cell.lblJobPost.text = value
                    }
                }
                
                if field_type_name == "job_status" {
                    if let value = innerDict["key"] as? String {
                        cell.lblJobType.text = value
                    }
                }
               
                if field_type_name == "view_resumes" {
                    if let value = innerDict["key"] as? String {
                        cell.lblBtnInactive.text = value
                    }
                }
        ...
        cell.btnInActive.addTarget(self, action: #selector(MatchedResumeViewController.nokri_btnInactiveClicked(_:)), for: .touchUpInside)
        return cell
    }
...
//        }
    ...
    @objc func nokri_btnInactiveClicked(_ sender: UIButton){
      ...
      let nextViewController = storyBoard.instantiateViewController(withIdentifier: "ViewResumeViewController") as! ViewResumeViewController
      nextViewController.job_id = sender.tag
    ...
    func nokri_activeJobData(){
        ...
        if UserDefaults.standard.bool(forKey: "isSocial") == true {
            ...
            print(param)
            Alamofire.request(Constants.URL.baseUrl+Constants.URL.matchedResume, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: headers)
    ...
    func nokri_activeJobDataPagination(){
        ...
        if UserDefaults.standard.bool(forKey: "isSocial") == true {
            ...
            print(param)
            Alamofire.request(Constants.URL.baseUrl+Constants.URL.matchedResume, method: .post, parameters: param, encoding: JSONEncoding.default, headers: headers)
                .responseJSON { response in
                    guard let res = response.value else{return}
                    let responseData = res as! NSDictionary
                    self.message = responseData["message"] as? String
    ...
    func nokri_filterRecvdResume(filterKey:Int){
        ...
        if UserDefaults.standard.bool(forKey: "isSocial") == true {
            ...
            print(param)
            Alamofire.request(Constants.URL.baseUrl+Constants.URL.matchedResumeFilter, method: .post, parameters: param, encoding: JSONEncoding.default, headers: headers)
                .responseJSON { response in
                    let responseData = response.value as! NSDictionary
                    let success = responseData["success"] as! Bool
                    self.message = responseData["message"] as? String
