//
//  SaveResumeViewController.swift
//  Nokri
//
...
class SaveResumeViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let obj = dataTabs.data.extra.saved_resumes
        self.title = obj
    }
      ...
      func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
          ...
          let cell = tableView.dequeueReusableCell(withIdentifier: "SaveResumeTableViewCell", for: indexPath) as! SaveResumeTableViewCell
          ...
          for itemDict in selectedFollower! {
              ...
              if let field_type_name = innerDict["field_type_name"] as? String{
                  if field_type_name == "candidate_name" {
                      if let value = innerDict["value"] as? String {
                          cell.lblName.text = value;
                      }
                  }
                  if field_type_name == "candidate_logo" {
                      if let value = innerDict["value"] as? String {
...
//                      }
//                  }
                  if field_type_name == "candidate_id" {
        ...
        let objData = NSKeyedUnarchiver.unarchiveObject(with: userData as! Data) as! [String: Any]
            ...
            //cell.btnRemove.setTitle(dataTabs.data.extra.remove_resume, for: .normal)
        }
        
        
      cell.btnRemove.addTarget(self, action: #selector(SaveResumeViewController.nokri_btnRemoveClicked(_:)), for: .touchUpInside)
      cell.btnViewProfile.addTarget(self, action: #selector(SaveResumeViewController.nokri_btnViewProfileClicked(_:)), for: .touchUpInside)
          return cell
      }
    ...
    func nokri_saveResumeData(){
        ...
        if UserDefaults.standard.bool(forKey: "isSocial") == true {
            ...
            if let userPassword = UserDefaults.standard.string(forKey: "password") {
                ...
                print(param)
                Alamofire.request(Constants.URL.baseUrl+Constants.URL.saveResume, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: headers)
...
//                        }
                        if let array = data["Resumes"] as? NSArray {
