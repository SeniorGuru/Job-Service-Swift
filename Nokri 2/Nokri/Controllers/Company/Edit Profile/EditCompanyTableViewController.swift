//
//  EditCompanyTableViewController.swift
//  Nokri
//
//  Created by Furqan Nadeem on 27/06/2020.
...
class EditCompanyTableViewController: UITableViewController,GMSMapViewDelegate,GMSAutocompleteViewControllerDelegate,CLLocationManagerDelegate,UITextFieldDelegate  {
...
//                       self.lblCountryKey.text = "\(jobCountryKey),\(jobStateKey),\(jobCityKey),\(jobTownKey)"
                    for jsobject in responseData["data"] as! [Dictionary<String, AnyObject>]{
                        let candlevel = jsobject["field_type_name"] as! String
                        if(candlevel == "cand_level"){
                            self.lblCountryKey.text = jsobject["key"] as! String
                            if let countryArr = jsobject["value"] as? NSArray {
                                   self.nokri_countryDataParser(countryArr: countryArr)
                            }
                        } else if(candlevel == "save"){
                            let buttontext = jsobject["value"] as! String
                            self.btnSaveLocation.setTitle(buttontext, for: .normal)
                        }
                    }
