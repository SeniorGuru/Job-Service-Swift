//
//  EditProfileCompanyInfoViewController.swift
//  Opportunities
//
...
class EditProfileCompanyInfoViewController: UITableViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        nokri_roundedImage()
        let tap: UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(EditProfileCompanyInfoViewController.dismisKeyboard as (EditProfileCompanyInfoViewController) -> () -> Void))
    ...
    @IBAction func btnSaveSection(_ sender: UIButton) {
        nokri_submitData()
    }
    
    @IBAction func txtFieldEditingBegin(_ sender: HoshiTextField) {
    }
    
    @IBAction func txtFieldEditingDidEnd(_ sender: HoshiTextField) {
    }
    
    @IBAction func profileImageClicked(_ sender: HoshiTextField) {
    ...
    func nokri_selectFromGalleryPressed(){
        ...
        if UIImagePickerController.isSourceTypeAvailable(.photoLibrary){
            ...
            let alert = UIAlertController(title: "Alert", message: "Gallery not available", preferredStyle: UIAlertController.Style.alert)
            let OkAction = UIAlertAction(title: "Ok", style: UIAlertAction.Style.cancel, handler: nil)
            alert.addAction(OkAction)
            self.present(alert, animated: true, completion: nil)
        }
    ...
    func nokri_customeButton(){
        ...
        btnSaveSection.layer.borderWidth = 1.0
        self.btnSaveSection.setTitleColor(UIColor(hex:appColorNew!), for: .normal)
        self.viewTop.backgroundColor = UIColor(hex: appColorNew!)
        btnSaveSection.layer.cornerRadius = 15
        richEditor.layer.borderWidth = 0.5
    ...
    func nokri_populateData() {
        ...
        for ac in extraArray{
            ...
            if ac.fieldTypeName == "del_acount"{
                btnDeleteAcc.setTitle(ac.value, for: .normal)
            }
        }
    }
    
    func nokri_submitData(){
        ...
        guard let headline = txtHeadline.text else {
            return
        }
        guard let web = txtWeb.text else {
            return
        }
        let param: [String: Any] = [
            // "email": email,
            "emp_name": name,
            "emp_phone": phone,
            "emp_headline": headline,
            "emp_intro": richEditor.text!,
            "emp_web": web,
            "emp_prof_stat":status
        ]
        print(param)
        self.nokri_basicInfoPost(parameter: param as NSDictionary)
    }
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        ...
        if textField == txtHeadline {
            txtHeadline.nokri_updateBottomBorderColor(isTextFieldSelected: true)
        } else {
            txtHeadline.nokri_updateBottomBorderColor(isTextFieldSelected: false)
        }
        if textField == txtImage {
            txtImage.nokri_updateBottomBorderColor(isTextFieldSelected: true)
        } else {
            txtImage.nokri_updateBottomBorderColor(isTextFieldSelected: false)
        }
        return true
    }
    ...
    func nokri_basicInfoPost(parameter: NSDictionary) {
