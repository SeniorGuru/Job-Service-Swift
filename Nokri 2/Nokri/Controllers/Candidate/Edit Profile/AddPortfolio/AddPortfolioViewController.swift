...
//  Opportunities
//
//  Created by Furqan Nadeem on 4/28/18.
...
class AddPortfolioViewController: UITableViewController,OpalImagePickerControllerDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout, UINavigationControllerDelegate,UITextFieldDelegate {
    ...
    func nokri_candPortfolioGet() {
        ...
        UserHandler.nokri_canPortfolioGet(success: { (successResponse) in
            self.stopAnimating()
    ...
    func nokri_uploadImages(images:[UIImage]) {
        ...
        UserHandler.nokri_uploadImageArr(fileName: "File", images:images, progress: { (uploadProgress) in
    ...
    func nokri_portfolioDelete(parameter: NSDictionary) {
        self.showLoader()
        UserHandler.nokri_canPortfolioDel(parameter: parameter as NSDictionary, success: { (successResponse) in
            self.stopAnimating()
    ...
    func nokri_SaveVideo(parameter: NSDictionary) {
        self.showLoader()
        UserHandler.nokri_videoUrl(parameter: parameter as NSDictionary, success: { (successResponse) in
            self.stopAnimating()
            if successResponse.success == true{
